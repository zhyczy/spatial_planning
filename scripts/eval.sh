...
# ══════════════════════════════════════════════════════════════════════════════
#  *** CONFIGURE: set the dataset to evaluate ***
...
INSTR_FILE="${INSTR_ROOT}/${PLANNING_MODEL_NAME}/results.jsonl"
if [[ ! -f "$INSTR_FILE" ]]; then
    echo "ERROR: Step-2 output not found for planning model: ${INSTR_FILE}"
    echo "       Check model name/path alignment between step2 and step3."
    exit 1
fi

conda run --no-capture-output -n spi python -u video_generation.py \
    --planning_model "$PLANNING_MODEL_NAME" \
    --predicted_instructions_root "$INSTR_ROOT" \
    --vace_ckpt      "$VACE_CKPT" \
    ${EVAL_LIMIT_ARG} \
...
# ══════════════════════════════════════════════════════════════════════════════
GEN_DIR="generated_videos/${DATASET}/${PLANNING_MODEL_NAME}/$(basename "$VACE_CKPT")"
