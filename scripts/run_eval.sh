...
#
#  Quick start:
#    bash scripts/run_eval.sh                               # smoke, qwen3.5 pretrained
#    bash scripts/run_eval.sh smoke                         # 10 samples, fast sanity check
#    bash scripts/run_eval.sh full                          # all samples
#    bash scripts/run_eval.sh smoke qwen3.5                 # pretrained Qwen3.5
#    bash scripts/run_eval.sh smoke qwen3-vl                # pretrained Qwen3-VL
#
#  Arguments:
...
#
#  Examples:
#    bash scripts/run_eval.sh smoke qwen3.5 false false   # our method + baseline, no thinking
#    bash scripts/run_eval.sh smoke qwen3.5 false true    # baseline WITH thinking (experiment 1)
#    bash scripts/run_eval.sh smoke qwen3.5 false false   # baseline WITHOUT thinking (experiment 2)
#
...
conda run --no-capture-output -n spi python -u generate_image_instructions.py \
    ...
    ${EVAL_LIMIT_ARG} \
    ${PLANNER_THINKING_ARG} \
    2>&1 | tee /tmp/eval_only_step2_${SCALE}.log
...
conda run --no-capture-output -n spi python -u evaluation.py \
    ...
    2>&1 | tee /tmp/eval_only_step4_${SCALE}.log
