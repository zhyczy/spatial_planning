...
(no coord head, no pose regression). The script supports four
...
inside the 4D M-RoPE (only active for the default mode; no effect on
...
- **Mutex**: `--polar` is mutually exclusive with `--decouple` (both imply
  the decouple architecture) and with `--vanilla`.
...
### 2.4 `--decouple` (Decoupled XYZ RoPE, Cartesian input)
...
### 2.5 `--interleave_vision` (orthogonal modifier)
...
- **Applies to**: default (4D M-RoPE)
- **No effect on**: `--vanilla` (3D M-RoPE), `--decouple` / `--polar` (Qwen
  original rotary layout untouched)
...
| | `--vanilla` | `--polar` | `--decouple` | `--interleave_vision` |
|---|:---:|:---:|:---:|:---:|
| `--vanilla` | — | ✗ | ✗ | ✗ (no-op) |
| `--polar` | | — | ✗ | ✗ (no-op) |
| `--decouple` | | | — | ✗ (no-op) |
...
Raised `ValueError`:
- `--decouple` with any of `--vanilla` / `--polar`
- `--polar` with any of `--vanilla` / `--decouple`
...
`AnswerOnlyModel` extracts logits at valid answer positions and computes
this loss.
...
- **Data**: `Eval_Dataset_Coord` (loads pts3d) → batches include `image_xyz`
...
loads per-patch xyz, and by dispatching the correct forward kwargs per mode.
...
```
correspondence_mindcube[_polar][_interleave][_decouple][_vanilla]
```
