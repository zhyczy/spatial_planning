...
call Qwen VL locally, and output a single video-generation instruction
that best aids the downstream reasoning model.
...
inside <think> and </think> tags. Analyze:
  ...
  - Would a video provide the most useful additional information? If so, what should it show?
  - Among all possible video descriptions, which single one would best help the downstream reasoning model?

Only after closing </think> should you output your instruction inside
<instructions>...</instructions> tags.
The single instruction goes inside an <instruction>...</instruction> tag.

Rules for the instruction:
  - If the existing images are sufficient, output empty <instructions></instructions>.
  - Otherwise output EXACTLY ONE instruction — the single video-generation description
    you believe will be most helpful to the downstream spatial reasoning model.
  - The instruction describes ONE specific video to generate (e.g., a camera path,
    a flythrough, a rotation, or a zoom that reveals the missing spatial information).
  - Focus on what visual information is missing. Typical needs include:
      * Viewpoint change: a camera movement to reveal hidden geometry
      * Occlusion removal: a trajectory that shows an object from an unblocked angle
    ...
    with open(output_path, "w") as out_f:
        for i, sample in enumerate(samples):
            ...
            record = {
                ...
                "instructions": instructions,   # List[str] of length 0 or 1; empty = no video needed
...
def main():
    ...
    config = tyro.cli(
        Config,
        description="Generate a single video-generation instruction from spatial QA datasets using Qwen VL (multi-GPU).",
    )
    ...
    # ── Resolve paths ──────────────────────────────────────────────────────────
    # File lives at spatial_planning/instruction_generation.py, so parent = spatial_planning/
